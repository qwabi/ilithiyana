...
export function SubjectStrip() {
  ...
  return (
    <section className='bg-primary-light py-20 font-sans overflow-hidden'>
      <div className='container mx-auto px-4'>
        ...
        <div className='mb-10'>
          ...
          <p className='mt-2 text-muted-foreground'>{gradeRange} — specialist support in the subjects learners need most.</p>
        </div>

        {/* Subject cards — horizontal scroll on mobile, grid on desktop */}
        <div className='flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory md:grid md:grid-cols-5 md:overflow-visible md:pb-0'>
          {subjectCards.map(({ name, Icon, bg, text, grades: g }) => (
            <div
              key={name}
              className={`min-w-[160px] snap-start flex-shrink-0 flex flex-col gap-3 rounded-2xl p-5 ${bg} md:min-w-0`}
            >
              <div className='flex h-12 w-12 items-center justify-center rounded-xl bg-white/20'>
                <Icon className={`h-6 w-6 ${text}`} aria-hidden />
              </div>
              <p className={`font-bold ${text} text-base leading-tight`}>{name}</p>
