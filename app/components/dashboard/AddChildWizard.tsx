...
import { portalSubjects } from '@/lib/validations/add-child';
...
async function uploadFile(file: File, leadId: string, purpose: 'report' | 'proof') {
...
export function AddChildWizard() {
  ...
  const handlePay = () => {
    if (!reportFile) {
      toast.error('School report is required');
      return;
    }
    ...
    startTransition(async () => {
      try {
        ...
        const report = await uploadFile(reportFile, leadId, 'report');
  ...
  return (
    <div className='mx-auto max-w-3xl space-y-6'>
            ...
            <Button
              onClick={() => {
                if (
                  ...
                  toast.error('Complete all learner fields');
                  return;
                }
                setStep(3);
              }}
      ...
      {step === 3 && (
        <div className='space-y-4 rounded-xl border border-border bg-white p-6'>
          <div>
            <Label>Latest school report (PDF or image)</Label>
            <Input
              type='file'
              accept='.pdf,image/*'
              className='mt-1'
